...
def screen_1ph():
    ...
    def hitung_kapasitor():
        try:
            ...
            pfAkhir = des_cosphi_entry.get()
            frequency = float(freq_entry.get())
            if pfAkhir == '-' :
                cos= 0.95
                kapasitor_perbaikan = calculate_capacitor_correction_1fase(voltage, current, pfAwal, cos, frequency)
                hasil.configure(text=f"Kapasitor perbaikan yang dibutuhkan\ndari tegangan 1 fase adalah: {round(kapasitor_perbaikan, 2)} μF")
            elif pfAkhir != '-' :
                cos= float(des_cosphi_entry.get())
                kapasitor_perbaikan = calculate_capacitor_correction_1fase(voltage, current, pfAwal, cos, frequency)
                hasil.configure(text=f"Kapasitor perbaikan yang dibutuhkan\ndari tegangan 1 fase adalah: {round(kapasitor_perbaikan, 2)} μF")
            else:
    ...
    title1=ctk.CTkLabel(inst_1ph,text='1 Phase',text_color='#66A4AC',font=('Bernard MT Condensed',20),justify='center',width=20,height=25)
    title1.place(x=190,y=30)
...
def screen_3ph():
    ...
    title.place(x=100,y=2)
    title1=ctk.CTkLabel(inst_3ph,text='3 Phase',text_color='#66A4AC',font=('Bernard MT Condensed',20),justify='center',width=20,height=25)
    title1.place(x=190,y=30)
...
def show_info():
    info ="Program ini menghitung koreksi kapasitor yang diperlukan untuk koreksi faktor daya.\n\n" \
            "1. Mulai aplikasi dengan mengeklik tombol 'Start'.\n"\
            "2. Pilih apakah Anda ingin menghitung sistem satu fasa atau tiga fasa.\n" \
            "3. Masukkan nilai yang diperlukan (tegangan, arus, faktor daya awal, faktor daya yang diinginkan, frekuensi).\n" \
            "4. Anda dapat mengisi kolom pada desired cosphi dengan tanda strip (-), jika tidak ingin menentukan nilai faktor daya akhir.\n" \
            "5. Klik tombol 'Hitung' untuk mendapatkan nilai koreksi kapasitor dalam mikro farad(uF).\n" \
            "6. Hasilnya akan ditampilkan di bawah tombol 'Hitung'.\n"
...
#By
by1=ctk.CTkLabel(main,text='Produced By :',font=('Arial',10),text_color='white',fg_color='transparent')
by1.place(x=185,y=150)
by2=ctk.CTkLabel(main,text='Arief Indra Kusuma (21635)',font=('Arial',10),text_color='white',fg_color='transparent')
by2.place(x=150,y=170)
by3=ctk.CTkLabel(main,text='Estu Bekti Cahyana (20706)',font=('Arial',10),text_color='white',fg_color='transparent')
by3.place(x=150,y=190)
by4=ctk.CTkLabel(main,text='Muhandis Lawdzai Putra Sanjaya (20992)',font=('Arial',10),text_color='white',fg_color='transparent')
